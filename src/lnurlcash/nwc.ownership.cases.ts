...
describe('service ownership', () => {
  it('subscribes only current-owner records and leaves foreign budgets untouched', async () => {
    ...
    const service = await startService(LINKING_KEY, {
      ...
      allocateOutputSecrets: () => Promise.resolve([]),
      mintSignatureKeys: () => [],
      recoverPendingMints: () => Promise.resolve(),
      reserveMintOutputSource: () => Promise.resolve(),
      finalizePaymentReturn: () => Promise.resolve(),
      getBearers: () => [],
      getDefaultMint: () => null,
      persistMintOutput: () => Promise.reject(new Error('unused')),
      discardMintOutput: () => Promise.reject(new Error('unused')),
      setMintOutputRetirement: () => Promise.reject(new Error('unused')),
      finalizeMintOutput: () => Promise.reject(new Error('unused')),
      finalizeSpentMintOutput: () => Promise.reject(new Error('unused')),
      commitCarve: () => Promise.reject(new Error('unused')),
