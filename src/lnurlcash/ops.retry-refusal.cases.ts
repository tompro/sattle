...
import {claimFromPreimage, prepareMint} from './ops/mint'
...
describe('mutation retry refusals', () => {
  it('rescues the outputs when the split answer is a retry refusal', async () => {
    ...
    const result = await ensureExactAmount([bearer], 25_000, {fetch: retryingCbFetch()})
  ...
  it('rescues the combined note when the merge answer is a retry refusal', async () => {
    ...
    const result = await ensureExactAmount([a, b], 25_000, {fetch: retryingCbFetch()})
  ...
  it('adopts the fresh secret when the claim rotate answer is a retry refusal', async () => {
    ...
    const claimed = await claimFromPreimage(prepared, preimage, {fetch: retryingCbFetch()})
  ...
  it('still fails a genuine refusal - a note spent before the split', async () => {
    ...
    await expect(
      ensureExactAmount([bearer], 25_000, {fetch: retryingCbFetch()}),
