...
const materialHash = (serialized: string): string => bytesToHex(sha256(utf8ToBytes(serialized)));
...
describe('saved wallet material commitment', () => {
  ...
  it('requires exactly one lowercase 64-hex commitment at the storage boundary', () => {
    ...
    expect(
      isValidStoredWalletMaterial({ ...record, materialHash: record.materialHash.toUpperCase() }),
    ).toBe(false);
    expect(
      isValidStoredWalletMaterial({
        enc: record.enc,
  ...
  it('rejects malformed authenticated material after its correct commitment matches', async () => {
    ...
    await expect(decryptSavedWalletMaterial(PASSWORD)).rejects.toThrow('valid v2 wallet material');
  });
